...
rule "corLightOn"
when   
    Item Gateway_SoundSwitch changed from OFF to ON
then   
    sendCommand(Gateway_SoundSwitch, ON)
end
rule "corLightOff"
when   
    Item Gateway_SoundSwitch changed from ON to OFF
then   
    sendCommand(Gateway_SoundSwitch, OFF)
end
EOF
cat > /etc/openhab2/sitemaps/xiaomi.sitemap << EOF
sitemap xiaomi label="Xiaomi" {
	Frame {
        Switch item=Gateway_SoundSwitch icon="light"
    }
}
EOF
